...
class Para:
    def __init__(self, hd=0, lr=0, drop=0, out=96, seed=1, avg=False):
        self.seed = seed
        # 输入的维度
        self.input_dim = 7
        # 隐藏层特征的维度
        ...
        if avg:
            self.directory = f'./average-time{out}/hd{self.hidden_dim}lr{self.learning_rate}drop{self.drop}'
        else:
            self.directory = f'./modelpara-time{out}'
